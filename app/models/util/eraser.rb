module Util
  class Eraser
    ...
    def remove_pmid
      ...
      @tab = delimiters[:tab]
      File.open("public/remove_pmid.txt", "w+") do |f|
        mgr = Util::WikiDataManager.new
        self.zika_studies.each do |id|
          qcodes=mgr.qcodes_for_nct_id(id)
          case qcodes.size
          when 0
            puts "========================================================="
            puts "ALERT:  No entity in wikidata with nct_id: #{id}"
            puts "========================================================="
          when 1
            qcode=qcodes.first
            study=Study.where('nct_id=?', id).first
            study.study_references.each{ |ref| f << "#{new_line}-#{qcode}#{tab}P698#{tab}\"#{ref.pmid}\"" if ref.pmid }
          else
            puts "========================================================="
            puts "ALERT:  Multiple entities in wikidata with nct_id: #{id}"
            puts "========================================================="
          end
        end
      end
